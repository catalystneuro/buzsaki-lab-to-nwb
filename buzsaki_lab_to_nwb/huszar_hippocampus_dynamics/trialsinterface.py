...
class HuszarTrialsInterface(BaseDataInterface):
    ...
    def run_conversion(self, nwbfile: NWBFile, metadata: dict, stub_test: bool = False):
        ...
        nwbfile.add_trial_column(name="choice", description="An value of 0 or 1 representing whether the expected and visited arm of the trial matches", data=trial_info["choice"])
        nwbfile.add_trial_column(
            name="visited_arm", description="An integer value representing the visited arm of the trial", data=trial_info["visitedArm"]
        )
        nwbfile.add_trial_column(
            name="expected_arm", description="An integer value representing the expected arm of the trial", data=trial_info["expectedArm"]
        )
        nwbfile.add_trial_column(
            name="recordings", description="An integer value representing which recording this trial belongs to", data=trial_info["recordings"]
        )
