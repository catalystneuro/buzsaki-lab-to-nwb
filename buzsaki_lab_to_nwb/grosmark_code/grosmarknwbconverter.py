...
class GrosmarkNWBConverter(NWBConverter):
    ...
    def __init__(self, **input_args):
        ...
        xml_filepath = os.path.join(
            input_args["GrosmarkLFP"]["folder_path"], session_id + ".xml"
        )
        ...
        input_args.update(
            BuzsakiNoRecording=dict(
                timeseries=np.array(range(n_channels)), sampling_frequency=1
            )
        )
    ...
    def get_metadata(self):
        ...
        session_path = self.data_interface_objects["GrosmarkLFP"].input_args[
            "folder_path"
        ]
        ...
        cell_filepath = os.path.join(
            session_path, "{}.spikes.cellinfo.mat".format(session_id)
        )
        if os.path.isfile(cell_filepath):
            cell_info = loadmat(cell_filepath)["spikes"]

        celltype_mapping = {"pE": "excitatory", "pI": "inhibitory"}
        celltype_filepath = os.path.join(
            session_path, "{}.CellClass.cellinfo.mat".format(session_id)
        )
        if os.path.isfile(celltype_filepath):
            celltype_info = [
                str(celltype_mapping[x[0]])
